...
function registerMouse(ctx: Context, spec: GuestSpec): () => void {
  return ctx.tools.register(defineTool({
    ...
    description: [
      ...
      'mouse units, and what a unit is worth is up to the guest\'s driver. Measured on Windows 3.1 at',
      '1024×768, reading the emulator\'s own framebuffer:',
      '  `home` lands the pointer tip at (0, 0) — the very corner, not near it.',
      '  A movement of 3 units or more travels 2 screen pixels per unit, exactly, at every distance',
      '  tried from 3 to 200 units.',
      '  A movement of 1 or 2 units travels 1 pixel per unit. That is the driver\'s acceleration: it',
      '  doubles anything from 3 up, and the threshold sits between 2 and 3.',
      'So: pixels ÷ 2 = units, for anything six pixels or more. Below that, one unit is one pixel and',
      'you cannot ask for 3, 4 or 5 pixels in a single step — take two.',
      '',
      'To reach a known place, `home` first and count from (0, 0), then `move` by half the pixel',
      'offset you want, then screenshot and correct. Another guest may have another driver and',
      'another ratio; re-measure the same way rather than assuming this one.',
      '',
