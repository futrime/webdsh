...
function dosDescription(spec: GuestSpec): string {
  return [
    ...
    `What is installed: ${spec.contains}`,
    '',
    'How this reads the output: the DOS console is moved onto the serial port with `CTTY COM1`, so',
    'what comes back is the command\'s whole output rather than the last 25 lines of it. While the',
    'console is there the machine\'s screen is frozen and its keyboard is ignored — `vm_screen`,',
    '`vm_type` and `vm_key` put it back automatically before they act, so mixing them costs a',
    'second and nothing else.',
    '',
    'A full-screen program — `vim`, `edit`, a game — will not work through this tool: it draws to a',
    'screen that is not connected. Run those with `vm_type` and watch them with `vm_screenshot`.',
